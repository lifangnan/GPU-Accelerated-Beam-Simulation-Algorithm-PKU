...
__global__ 
void SimulateStraightCapillaryKernel(double* r_x, double* r_y, double* r_phi, 
  double* r_xp, double* r_yp, double* r_w, uint* r_loss, double r_length, 
  double r_aper, double r_current, uint r_elem_indx)
{
  double aper2 = r_aper;
  aper2 *= aper2;
  uint index = blockIdx.x * blockDim.x + threadIdx.x;
  uint stride = blockDim.x * gridDim.x;
  // double qg = r_field * d_const.charge;
  double j = r_current / PI / aper2;
  double k_up = d_const.charge * 4e-7 * PI * j * CLIGHT;
  ...
  while(index < d_const.num_particle)
  {
    if(r_loss[index] == 0)
    {
      ...
      //   theta = -PI / 2;
      // }
      // double thetap = (-xp*y + yp*x) / rhop / rhop;

      // double new_rho, new_rhop, new_theta, new_thetap;
      

      double gamma = w / d_const.mass + 1.0; 
      double beta = std::sqrt(1.0 - 1.0 / (gamma * gamma));
      double k = sqrt(k_up / 2.0 / d_const.mass / beta / gamma);
      double Phi = r_length * k;

      double cosPhi = cos(Phi);
      double sinPhi = sin(Phi);
      // double sinTheta = y / rho;
      // double cosTheta = x / rho;

      // new_rho = cosPhi * rho + sinPhi/k * rhop;
      // new_rhop = -k * sinPhi * rho + cosPhi * rhop;
      
      r_x[index] = cosPhi * x + sinPhi/k * xp;
      r_xp[index] = -k * sinPhi * x + cosPhi * xp;
      r_y[index] = cosPhi * y + sinPhi/k * yp;
      r_yp[index] = -k * sinPhi * y + cosPhi * yp;
...
__global__ 
void SimulateCurvedCapillaryKernel(double* r_x, double* r_y, double* r_phi, 
  double* r_xp, double* r_yp, double* r_w, uint* r_loss, double r_angle, double r_radius,
  double r_aper, double r_current, uint r_elem_indx)
{
  double aper2 = r_aper;
  aper2 *= aper2;
  ...
  while(index < d_const.num_particle)
  {
    if(r_loss[index] == 0)
    {
      ...
      double beta = std::sqrt(1.0 - 1.0 / (gamma * gamma));
      double k = sqrt(K_up / 2.0 / d_const.mass / beta / gamma);
      double r_length = abs(r_angle * r_radius);
      double Phi = r_length * k;
      double eta = 1.0/k/k/r_radius;

      // angle > 0: bend to left
      if(r_angle < 0){
        eta = -eta;
      }
    ...
    // at the center & the end of the quad, check for loss
      if(aper2 != 0 && (x+eta) * (x+eta) + y * y > aper2)
        r_loss[index] = r_elem_indx;
      else // if not lost, then check phase 
      {
        double gm1 = w / d_const.mass;
