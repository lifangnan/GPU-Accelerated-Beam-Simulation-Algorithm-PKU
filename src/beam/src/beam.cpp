...
void Beam::UpdateAvgX(bool r_good_only)
{
  ...
  // StopTimer(&start, &stop, "Update AvgX: ");
}
...
// Edited by Fangnan Li
double Beam::GetAvgXp() const
{
  return GetDataFromDevice(xp_avg, 0);
}

double Beam::GetAvgYp() const
{
  return GetDataFromDevice(yp_avg, 0);
}

double Beam::GetSigXp() const
{
  return GetDataFromDevice(xp_sig, 0);
}

double Beam::GetSigYp() const
{
  return GetDataFromDevice(yp_sig, 0);
}
